...
LOG_DIR="${PROJECT_ROOT}/logs/xian_cloud"
CHECKPOINT_DIR="${PROJECT_ROOT}/checkpoints/xian_cloud"
...
if [[ -f "${PID_FILE}" ]]; then
    ...
    if [[ "${existing_pid:-}" =~ ^[0-9]+$ ]] && kill -0 "${existing_pid}" 2>/dev/null; then
        echo "Xi'an-cloud training is already running with PID ${existing_pid}."
        exit 1
    fi
fi
...
echo "Xi'an-cloud training started."
echo "PID: ${training_pid}"
echo "Console log: ${console_log}"
echo "Structured log: ${LOG_DIR}/train.log"
echo "Best checkpoint: ${CHECKPOINT_DIR}/xian_cloud_best_miou.pt"
echo "Last checkpoint: ${CHECKPOINT_DIR}/xian_cloud_last.pt"
