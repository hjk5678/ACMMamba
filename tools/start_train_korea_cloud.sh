...
LOG_DIR="${PROJECT_ROOT}/logs/korea_cloud"
CHECKPOINT_DIR="${PROJECT_ROOT}/checkpoints/korea_cloud"
...
if [[ -f "${PID_FILE}" ]]; then
    ...
    if [[ "${existing_pid:-}" =~ ^[0-9]+$ ]] && kill -0 "${existing_pid}" 2>/dev/null; then
        echo "Korea-cloud training is already running with PID ${existing_pid}."
        exit 1
    fi
fi
...
setsid "${PYTHON_BIN}" -m torch.distributed.run \
    ...
    train.py \
    --config configs/train_korea_cloud.yaml \
    > "${console_log}" 2>&1 < /dev/null &

training_pid=$!
echo "${training_pid}" > "${PID_FILE}"

echo "Korea-cloud training started."
echo "PID: ${training_pid}"
echo "Console log: ${console_log}"
echo "Structured log: ${LOG_DIR}/train.log"
echo "Best checkpoint: ${CHECKPOINT_DIR}/korea_cloud_best_miou.pt"
echo "Last checkpoint: ${CHECKPOINT_DIR}/korea_cloud_last.pt"
