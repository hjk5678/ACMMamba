...
LOG_DIR="${PROJECT_ROOT}/logs/kust4k"
CHECKPOINT_DIR="${PROJECT_ROOT}/checkpoints/kust4k"
...
if [[ -f "${PID_FILE}" ]]; then
    ...
    if [[ "${existing_pid:-}" =~ ^[0-9]+$ ]] && kill -0 "${existing_pid}" 2>/dev/null; then
        echo "Kust4K training is already running with PID ${existing_pid}."
        exit 1
    fi
fi
...
echo "Kust4K training started."
echo "PID: ${training_pid}"
echo "Console log: ${console_log}"
echo "Structured log: ${LOG_DIR}/train.log"
echo "Best checkpoint: ${CHECKPOINT_DIR}/kust4k_best_miou.pt"
echo "Last checkpoint: ${CHECKPOINT_DIR}/kust4k_last.pt"
