...
LOG_DIR="${PROJECT_ROOT}/logs/bjroad"
CHECKPOINT_DIR="${PROJECT_ROOT}/checkpoints/bjroad"
...
if [[ -f "${PID_FILE}" ]]; then
    ...
    if [[ "${existing_pid:-}" =~ ^[0-9]+$ ]] && kill -0 "${existing_pid}" 2>/dev/null; then
        echo "BJRoad training is already running with PID ${existing_pid}."
        exit 1
    fi
fi
...
setsid "${PYTHON_BIN}" -m torch.distributed.run \
    ...
    train.py \
    --config configs/train_bjroad.yaml \
    > "${console_log}" 2>&1 < /dev/null &

training_pid=$!
echo "${training_pid}" > "${PID_FILE}"

echo "BJRoad training started."
echo "PID: ${training_pid}"
echo "Console log: ${console_log}"
echo "Structured log: ${LOG_DIR}/train.log"
echo "Best checkpoint: ${CHECKPOINT_DIR}/bjroad_best_miou.pt"
echo "Last checkpoint: ${CHECKPOINT_DIR}/bjroad_last.pt"
